...
- Run `setup.sql`, then `seed_products.sql` (the extra catalogue: every category ends up with 50+ products,
  256 of the 264 come with a real photo already, every one of them with a verified photo credit so the
  (i) button on its product page works; the other 8 show a category placeholder until you upload one in
  Admin > Products). If you already ran an older `seed_products.sql` without photos, run
...
- In a hurry to see it live? Run `seed_products_quickstart.sql` instead: 30 products per category
  (150 total), all with a real, credited photo. You can run `seed_products.sql` afterwards for the rest.
